...
void loop()
{
  ...
  if (checkForBeat(irValue) == true)
  {
    ...
    if (beatsPerMinute < 255 && beatsPerMinute > 20)
    {
      ...
      // Toggle water pump based on heart rate threshold
      if (beatAvg > 70) // Adjust the threshold as needed
      {
        digitalWrite(relayPin, HIGH); // Turn ON the water pump
        Serial.println("Heart rate exceeds threshold. Water pump is ON");
      }
      else
      {
        digitalWrite(relayPin, LOW); // Turn OFF the water pump
        Serial.println("Heart rate below threshold. Water pump is OFF");
      }
    }
  }
